...
bool http_conn::write(){
    ...
    while(1){
        ...
        if(temp <= -1){
            ...
            if(errno == EAGAIN){  
                modfd(m_epollfd, m_sockfd, EPOLLOUT);  // https://blog.csdn.net/CODINGCS/article/details/115046256
                return true;
            }
            unmap();
            return true;
        ...
        if(bytes_to_send <= bytes_have_send){
            ...
            unmap();  // 全部写完之后要释放掉
            if(m_linger){   // TODO 为什么保持连接的话，要初始化一下？
                init();
                modfd(m_epollfd, m_sockfd, EPOLLIN);  // 缓冲区可读，所以更改状态为EPOLLIN
                return true;
            }else{
                modfd(m_epollfd, m_sockfd, EPOLLIN);
...
// 往写缓冲中写入待发送的数据
bool http_conn::add_response(const char * format, ...){ // 后面是可变参数
