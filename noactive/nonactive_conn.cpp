...
void sig_handler(int sig){  // 处理信号
    ...
    int msg = sig;
    send(pipefd[1], (char *)&msg, 1, 0);    // 往pipefd[1]中写, 0表示读
    errno = save_errno;
}
...
int main(int argc, char* argv[]){
    ...
    bool timeout = false;
    alarm(TIMESLOT); // 定时,5s后产生SIFGALARM信号
    ...
    while(!stop_server){
        ...
        for(int i = 0 ; i < number ; i ++){
            ...
            if(sockfd == listenfd){
                ...
                // 创建定时器,设置其回调函数与超时时间,然后绑定定时器与用户数据,最后将定时器加入定时器链表中
                ...
                if(ret < 0){
                    ...
                    // 如果某个客户端上有数据可读,则我们要调整该连接对应的定时器,以延迟该
                    if(timer){
                        time_t cur = time(NULL);
                        timer->expire = cur + 3 * TIMESLOT;
        ...
        // 最后处理定时时间,因为I/O事件有更高的优先级.当然,这样做将导致定时任务不能精准的
        if(timeout){
            timer_handler();
            timeout = false;
        }
